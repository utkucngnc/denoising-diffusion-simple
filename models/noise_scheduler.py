...
class NoiseScheduler:
    def __init__(self, function = None, timesteps = None) -> None:
        ...
        self.betas = th.from_numpy(self.Gamma()).double()
    
    def Gamma(self):
        if self.function == "Linear":
            scale = 1000 / self.timesteps # 1000 is the default value for beta in the paper
            beta_start, beta_end = GetConfig(key="Noise Function")[self.function].values()
            ...
            start, end, tau = GetConfig(key="Noise Function")[self.function].values()
